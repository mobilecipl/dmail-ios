...
- (void)animateDismissal:(id<UIViewControllerContextTransitioning>)transitionContext
{
	...
	[UIView animateKeyframesWithDuration:0.3 delay:0.0 options:UIViewKeyframeAnimationOptionCalculationModeCubic animations:^{
		[UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:1.0 animations:^{
            // Move source view below source view
            source.view.frame = [self presentingControllerFrameWithContext:transitionContext];
		}];
		[UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:1.0 animations:^{
			// Important: original transform3d is different from CATransform3DIdentity
			CATransform3D newPerspectiveTransform = source.view.layer.transform;
			
			newPerspectiveTransform.m34 = 1.0 / -1000.0;
			newPerspectiveTransform = CATransform3DTranslate(newPerspectiveTransform, 0, 0, 0);
			destination.view.layer.transform = newPerspectiveTransform;
		}];
	} completion:^(BOOL finished) {
